...
namespace FileManagement
{
    ...
    class Simple_Code
    {
        ...
        public static Dictionary<string, List<Worker>> DictionaryList(ref List<Worker> workers)
        {
            ...
            List<string> listOfDesignation = workers
                           .Select(x => x.Designation)
                           .Where(designation => !string.IsNullOrEmpty(designation))
                           .Distinct().ToList();
                            //Important line - System.ArgumentNullException: 'Value cannot be null. (Parameter 'key')' - withour where statement.
